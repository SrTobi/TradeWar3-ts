/**
 * Shared Game Engine - Core game logic used by both server and P2P host
 * This module contains all game state management, player handling, and game loop logic
 */

import type { GameState, Player, Country, Company, Faction, HexCoord } from '@/types/game';
import type { ClientMessage, ServerMessage, GameInfo } from '@/network/messages';
import { generateHexGrid } from '@/game/hex';
import { processBattle, calculateUnitCost, canPlaceUnits, checkWinner } from '@/game/battle';
import { createCompanies, updateStockPrice } from '@/game/stock';
...
export const AI_ACTION_INTERVAL = 800; // ms between AI actions
...
/**
 * Transport interface - implemented differently by WebSocket server and P2P host
 */
...
export class GameEngine {
  ...
  private aiCounter: number;
  private onGameListChanged?: () => void;

  constructor(
    transport: GameTransport,
    clients: Map<string, GameClient>,
    games: Map<string, GameRoom>,
    counters: { playerCounter: number; gameCounter: number; aiCounter: number }
  ) {
    this.transport = transport;
    this.clients = clients;
    this.games = games;
    this.playerCounter = counters.playerCounter;
    this.gameCounter = counters.gameCounter;
    this.aiCounter = counters.aiCounter;
  }

  setOnGameListChanged(callback: () => void): void {
  ...
  handleMessage(playerId: string, msg: ClientMessage): void {
    ...
    switch (msg.type) {
      ...
      case 'placeUnits':
        ...
        break;
      case 'ping':
        this.transport.send(client.playerId, { type: 'pong', timestamp: msg.timestamp });
        break;
      case 'addAi':
  ...
  private handleListGames(client: GameClient): void {
    ...
    this.transport.send(client.playerId, { type: 'gameList', games });
  }

  private handleCreateGame(client: GameClient): void {
    if (!client.playerName) {
      this.transport.send(client.playerId, { type: 'error', message: 'Set your name first' });
      return;
    }
    ...
    this.transport.send(client.playerId, { type: 'joinedGame', gameId, factionId });
  ...
  private handleJoinGame(client: GameClient, gameId: string): void {
    if (!client.playerName) {
      this.transport.send(client.playerId, { type: 'error', message: 'Set your name first' });
      return;
    }
    ...
    if (!room) {
      this.transport.send(client.playerId, { type: 'error', message: 'Game not found' });
      return;
    }

    if (room.gameState?.phase === 'playing') {
      this.transport.send(client.playerId, { type: 'error', message: 'Game already in progress' });
      return;
    }
  ...
  private handleLeaveGame(client: GameClient): void {
    ...
    this.transport.send(client.playerId, { type: 'leftGame' });
  ...
  private destroyGame(room: GameRoom): void {
    ...
    for (const playerId of room.clients) {
      ...
      if (c) {
        ...
        this.transport.send(c.playerId, { type: 'leftGame' });
      }
    }
  ...
  private handleStartGame(client: GameClient): void {
    if (!client.currentGameId) return;
    ...
    if (room.hostId !== client.playerId) {
      this.transport.send(client.playerId, { type: 'error', message: 'Only host can start the game' });
      return;
    }
    ...
    this.transport.broadcastToGame(room, { type: 'gameStarted' });
  ...
  private handlePlaceUnits(client: GameClient, coords: HexCoord): void {
    if (!client.currentGameId) return;
  ...
  private handleAddAi(client: GameClient): void {
    if (!client.currentGameId) return;
    ...
    if (room.hostId !== client.playerId) {
      this.transport.send(client.playerId, { type: 'error', message: 'Only host can add AI players' });
      return;
    }

    // Cannot add AI after game started
    if (room.gameState?.phase === 'playing') {
      this.transport.send(client.playerId, { type: 'error', message: 'Cannot add AI during game' });
      return;
    }
    ...
    if (totalPlayers >= 6) {
      this.transport.send(client.playerId, { type: 'error', message: 'Game is full' });
      return;
    }
  ...
  private startGameLoop(room: GameRoom): void {
    room.gameLoop = setInterval(() => {
      ...
      this.transport.broadcastToGame(room, { type: 'gameState', state: room.gameState });
  ...
  getGamePlayers(room: GameRoom): Player[] {
  ...
  private broadcastLobbyUpdate(room: GameRoom): void {
    const players = this.getGamePlayers(room);
    this.transport.broadcastToGame(room, { type: 'lobbyUpdate', players });
  }

  broadcastGameList(): void {
    if (this.onGameListChanged) {
